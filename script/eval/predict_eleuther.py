...
######## Models for SciLLM, SFT experiments ##########
LOCAL_MODELS = ["Qwen2-7B-Instruct",
                "SciTulu-7B-Instruct",
                "Llama3-8B-Instruct",
                "Mistral-7B-Instruct",
...
def make_parser():
    ...
    parser.add_argument(
        ...
        choices=["vllm", "openai-chat-completions", "anthropic",
                 "local-chat-completions", "azure-openai-chat-completions"],
    )
    parser.add_argument(
        "--model_name",
        type=str,
...
def make_task_command(task_name, result_dir, args):
    ...
    if args.model == "vllm":
        ...
        if args.base_url is None:
            ...
            assert args.model_name in LOCAL_MODELS, f"Model {args.model_name} not found in LOCAL_MODELS"
            model_args = f"model={args.model_name},base_url={args.base_url}"
    ...
    command = [
        ...
        model_args,
        "--gen_kwargs",
        "max_gen_toks=1024,do_sample=False,temperature=0",  # TODO(dwadden) Should make this modifiable. 
        "--tasks",
        task_name,
        "--batch_size",
        "1",
...
def kickoff(args):
    ...
    if not args.beaker:
        for task_spec in experiment_tasks:
            ...
            print("Command to run:", task_spec.command)  # Add this line to print the command
